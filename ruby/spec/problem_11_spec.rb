...
describe ProblemEleven do
  ...
  it "setup_grid should return array" do
    expect(@foo.setup_grid(@grid)).to be_kind_of(Array)
  end
  ...
  describe "find max prod" do
    ...
    it "should return the max product for all rows and columns" do
      ...
      expect(@foo.find_rows_and_cols_max_prod(grid)).to eq(14641)
    end

    it "should return the max product for all diags" do
      grid = @foo.setup_grid(@grid)
      expect(@foo.find_diag_max_prod(grid)).to eq(341)
    end
  end
end
...
grid = problem.setup_grid(string)
